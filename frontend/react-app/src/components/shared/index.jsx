import { FOODS, WEEKDAYS, L, tr } from '../../data/data';
...
export function Icon({ name, size = 20, stroke = 2, fill = false, style = {} }) {
...
export function Money({ amount, lang, suffix = false }) {
  if (amount === 0) return <span className="price-free">{L('free', lang)}</span>;
  return (
    <span className="price">
      <span className="rupee">₹</span>{amount}
      {suffix && <span className="price-suffix">{L('perPerson', lang)}</span>}
    </span>
  );
}

export function Chip({ active, children, onClick, emoji }) {
  return (
...
export function FoodTag({ foodKey, count, active, onClick, lang }) {
  const f = FOODS[foodKey];
  const dim = count === 0;
  const shown = useCountUp(count);
  return (
    <button
      className={'foodtag' + (active ? ' foodtag-on' : '') + (dim ? ' foodtag-dim' : '')}
      onClick={dim ? undefined : onClick}
      disabled={dim}>
      <span className="foodtag-emoji">{f.emoji}</span>
      <span className="foodtag-name">{tr(f, lang)}</span>
...
export function ActionBtn({ icon, label, variant = 'ghost', onClick }) {
  return (
...
export function Facility({ name, lang }) {
  ...
  return (
    <div className="facility">
      ...
      <span className="facility-label">{tr(f, lang)}</span>
    </div>
  );
}
...
export function BottomNav({ active, lang, onNav }) {
  const items = [
    ['home', 'pin', 'navHome'],
    ['map', 'map', 'navMap'],
